...
export default function LogPage() {
  ...
  const workoutsById = useMemo(() => {
    return new Map(
      (effectiveProgramme ? getAllWorkouts(effectiveProgramme) : []).map(
        (workout) => [workout.id, workout] as const,
      ),
    );
  }, [effectiveProgramme]);
    ...
    () =>
      logs.map((log) => {
        const resolvedCategory: ResolvedCategory =
          log.workoutCategory ??
          workoutsById.get(log.workoutId)?.category ??
          "unknown";
        ...
        return {
          ...
          resolvedSessionType:
            ...
            workoutsById.get(log.workoutId)?.sessionType,
          resolvedWorkoutDate:
            log.workoutDate ?? workoutsById.get(log.workoutId)?.date,
        };
      }),
    [logs, workoutsById],
  );
  ...
  const categoryFilters = useMemo(() => {
    return Array.from(
      new Set(logsWithCategory.map((log) => log.resolvedCategory)),
    );
  }, [logsWithCategory]);
  ...
  const sortedLogs = useMemo(
    () =>
      logsWithCategory
        .filter(
          (log) =>
            categoryFilter === "all" || log.resolvedCategory === categoryFilter,
        )
        .sort(
          (first, second) =>
          ...
          {sortedLogs.map(
            (
              log: SessionLog & {
                ...
                resolvedSessionType?: string;
                resolvedWorkoutDate?: string;
              },
            ) => {
              ...
              return (
                <article key={log.id} className="tv-card p-4">
                  ...
                  {log.notes ? (
                    <p className="mt-4 break-words text-sm font-bold text-[var(--muted)]">
                      {log.notes}
                    </p>
                  ) : null}
                    ...
                    <details className="mt-4 rounded-md border border-[var(--border)] bg-black/60 p-3">
                      ...
                      <div className="mt-3 grid gap-2 text-sm font-bold">
                        {log.blockResults.map((blockResult) => (
                          <div
                            key={blockResult.blockKey}
                            className="grid gap-1 border-t border-[var(--border)] pt-2 sm:grid-cols-[10rem_minmax(0,1fr)]"
                          >
                            <span className="break-words font-black uppercase text-[var(--text)]">
                              {blockResult.blockName}
                            </span>
                            <span className="break-words text-[var(--muted)]">
                              {getBlockResultSummary(blockResult)}
                            </span>
                          </div>
                        ))}
                      </div>
