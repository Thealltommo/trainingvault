...
test("reconciles a moved session using the athlete-confirmed completion time", () => {
  const result = matchGarminActivity(
    activity({
      title: "Carlisle - Long aerobic run",
      startTime: "2026-08-03T11:01:00Z",
      localStartTime: "2026-08-03T12:01:00",
      durationSeconds: 3_782,
      movingDurationSeconds: 3_760,
      distanceMeters: 10_970,
    ...
    [
      {
        sessionId: "long-aerobic-run",
        title: "Long aerobic run",
        // The plan was subsequently moved, so its current calendar date no
        // longer describes when the athlete actually completed the workout.
        date: "2026-08-06",
        completedAt: "2026-08-03T14:08:00+01:00",
        plannedDurationSeconds: 3_840,
      },
    ],
  );

  assert.equal(result.kind, "matched");
  assert.equal(result.candidate.sessionId, "long-aerobic-run");
  assert.ok(result.candidate.reasons.includes("completion_date"));
  assert.ok(result.candidate.reasons.includes("title"));
  assert.ok(result.candidate.reasons.includes("duration"));
});
